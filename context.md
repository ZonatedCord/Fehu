...
| 2026-07-27 | Fix modale "Rettifica saldo" Dashboard: errore "invalid type: null, expected f64" su `create_balance_adjustment`. Root cause: `bind:value` su `<input type="number">` in Svelte imposta `null` (non `0`/`NaN`) quando il campo viene svuotato; le guardie in `Dashboard.svelte` controllavano solo `rettAmount === 0`, lasciando passare `null` fino al comando Tauri che richiede `f64` non-opzionale. Fix: guardie cambiate a check falsy (`!rettAmount`), tipo state `number \| null`. Rilasciato v0.2.12. |
